...
<?
	for($i = 0; $i < $cnt; $i++):
?>
	BX_YMapAddPlacemark(<?echo CUtil::PhpToJsObject($arResult['POSITION']['PLACEMARKS'][$i])?>, '<?echo $arParams['MAP_ID']?>');
<?
	endfor;
?>
}
...
if (window.attachEvent) // IE
	window.attachEvent("onload", BXShowMap_<?echo $arParams['MAP_ID']?>);
else if (window.addEventListener) // Gecko / W3C
	window.addEventListener('load', BXShowMap_<?echo $arParams['MAP_ID']?>, false);
else
	window.onload = BXShowMap_<?echo $arParams['MAP_ID']?>;
</script>
<?endif;?>