...
if (!defined('BX_GMAP_SCRIPT_LOADED'))
{
	...
	{
		$APPLICATION->AddHeadString('<script src="http://maps.google.com/maps?file=api&v='.intval($arParams['GOOGLE_VERSION']).'&key='.$arParams['KEY'].'" charset="utf-8"></script>');
...
$arResult['ALL_MAP_TYPES'] = array('NORMAL', 'SATELLITE', 'HYBRID');
$arResult['ALL_MAP_OPTIONS'] = array('ENABLE_SCROLL_ZOOM' => 'ScrollWheelZoom', 'ENABLE_DBLCLICK_ZOOM' => 'DoubleClickZoom', 'ENABLE_DRAGGING' => 'Dragging', 'ENABLE_CONT_ZOOOM' => 'ContinuousZoom', 'ENABLE_GOOGLE_BAR' => 'GoogleBar');
$arResult['ALL_MAP_CONTROLS'] = array('LARGE_MAP_CONTROL' => 'LargeMap', 'SMALL_MAP_CONTROL' => 'SmallMap', 'SMALL_ZOOM_CONTROL' => 'SmallZoom', 'MINIMAP' => 'OverviewMap', 'TYPECONTROL' => 'MapType', 'HTYPECONTROL' => 'HierarchicalMapType', 'SCALELINE' => 'Scale');

if (!$arParams['INIT_MAP_TYPE'] || !in_array($arParams['INIT_MAP_TYPE'], $arResult['ALL_MAP_TYPES']))
	$arParams['INIT_MAP_TYPE'] = 'NORMAL';
...
if (!is_array($arParams['CONTROLS']))
	$arParams['CONTROLS'] = array('LARGE_MAP_CONTROL', 'HTYPECONTROL', 'MINIMAP', 'SCALELINE');
else
{
