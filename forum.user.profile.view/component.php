...
foreach ($arResult["FORUM_USER"] as $key => $val):
	...
	if (!is_array($val))
		$arResult["FORUM_USER"][$key] = $parser->wrap_long_words($val);
endforeach;
...
$arResult["SHOW_VOTES"] = ((COption::GetOptionString("forum", "SHOW_VOTES", "Y") == "Y" && $USER->IsAuthorized()
	&& ($USER->IsAdmin() || intVal($USER->GetParam("USER_ID"))!=$arParams["UID"])) ? "Y" : "N");
...
if ($arResult["SHOW_VOTES"] == "Y"):
	...
	elseif (intVal($arUserRank["VOTES"]) > 0 || $USER->IsAdmin())
	{
...
$arResult["SHOW_NAME"] = htmlspecialcharsEx(trim($arResult["USER"]["~NAME"]." ".$arResult["USER"]["~LAST_NAME"]));
...
if ($db_res && ($res = $db_res->GetNext())):
	...
	rsort($arUserPerm);
	$arResult["USER_RANK"] = (array_key_exists($arUserPerm[0], $ForumsPerms) ? $ForumsPerms[$arUserPerm[0]] : "");
endif;
if ($arResult["SHOW_RANK"] == "Y"):
	$arRank = CForumUser::GetUserRank($arParams["UID"], LANGUAGE_ID);
	$arResult["USER_RANK"] = (empty($arResult["USER_RANK"]) ? $arRank["NAME"] : $arResult["USER_RANK"]);
	$arRank["NAME"] = $arResult["USER_RANK"];
	$arResult["arRank"] = $arRank;
	$arResult["SHOW_POINTS"] = "N";
	if ($USER->IsAuthorized() && ($USER->IsAdmin() || intVal($USER->GetParam("USER_ID"))==$arParams["UID"]))
	{
		$arResult["SHOW_POINTS"] = "Y";
		$arResult["USER_POINTS"] = (intVal($arRank["VOTES"])>0 ? intVal($arRank["VOTES"]) : GetMessage("F_NO_VOTES"));
	}
endif;
$arResult["USER_RANK"] = (empty($arResult["USER_RANK"]) ? GetMessage("LU_USER_USER") : $arResult["USER_RANK"]);
/*******************************************************************/
$arResult["arTopic"] = "N";
