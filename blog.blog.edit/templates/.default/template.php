		...
		<tr>
			...
			<td>
				...
				function newGroup()
				{
					...
					html = document.getElementById('forNewGroup').innerHTML;
					newHtml = '<div id="group-line-'+id+'">' +
						'<input id="open_group_'+id+'" type="checkbox" name="group['+id+']" value="Y">' +
						'<span id="group-form-'+id+'" style="display:inline;"><input name="GROUP_NAME" id="group_name_'+id+'" size="20" maxlength="255" value=""> <input type="button" name="blog_save" value="Ok" onclick="javascript:saveGroup(\''+id+'\');"></span>'+
					...
					document.getElementById('forNewGroup').innerHTML = html + newHtml;
				...
				<table class="blog-edit-perms-table">
					...
					if(!empty($arResult["USER_GROUP"]))
					{
						foreach($arResult["USER_GROUP"] as $v)
						{
							...
							<tr>
								<td nowrap><?=$v['NAME']?></td>
								<td><?=ShowSelectPerms('p',$v['ID'],$arResult["BLOG"]["perms_p"][$v['ID']],$arResult["BLOG_POST_PERMS"])?></td>
								<td><?=ShowSelectPerms('c',$v['ID'],$arResult["BLOG"]["perms_c"][$v['ID']],$arResult["BLOG_COMMENT_PERMS"])?></td>
							</tr>
							<?
						}
					}
