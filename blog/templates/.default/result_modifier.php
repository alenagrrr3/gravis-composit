...
if($arParams["SHOW_NAVIGATION"] != "N" && (IntVal($arResult["VARIABLES"]["group_id"]) > 0 || strlen($arResult["VARIABLES"]["blog"]) > 0 || IntVal($arResult["VARIABLES"]["user_id"]) > 0))
{
	...
	if(strlen($arResult["PATH_TO_INDEX"])<=0)
		$arResultTmp["PATH_TO_INDEX"] = htmlspecialchars($GLOBALS['APPLICATION']->GetCurPage()."?".$arParams["PAGE_VAR"]."=index");	
	...
	if(IntVal($arParams["GROUP_ID"]) <= 0)
	{
		...
		elseif(strlen($arResult["VARIABLES"]["blog"]) > 0)
		{
			if($arBlog = CBlog::GetByUrl($arResult["VARIABLES"]["blog"]))
			{
				if($arBlog["ACTIVE"] == "Y")
				{
	...
	if(strlen($arResult["VARIABLES"]["blog"]) > 0 || IntVal($arResult["VARIABLES"]["user_id"]) > 0)
	{
		...
		if(empty($arBlog))
		{
			$arBlog = CBlog::GetByOwnerID($arResult["VARIABLES"]["user_id"]);
		}
