...
?><a href="<?=CComponentEngine::MakePathFromTemplate($arParams["SECTIONS_TOP_URL"], array())?>" title="<?=GetMessage("P_UP_TITLE")?>"  class="photo-action back-to-album" <?
	?>><?=GetMessage("P_UP")?></a><?
?></div><?
...
?><div class="photo-controls photo-view only-on-main"><?
	...
	foreach ($arSort as $key => $val):
		...
		?><a href="<?=$GLOBALS['APPLICATION']->GetCurPageParam("&order=".$key, array("order"))?>" <?
			?>title="<?=$val["description"]?>" class="photo-view <?=$key?><?=$addClassName?>"><?=$val["title"]?></a><?
...
?><form action="<?=POST_FORM_ACTION_URI?>"<?
	?> id="photo_filter_form" method="post"><?
...
?></form><?
?></div><?
?><a href="javascript:void(0);" onclick="show_filter()" class="photo-action set-filter"><?=GetMessage("P_SET_FILTER")?></a><?
?></div><br />
<script>
