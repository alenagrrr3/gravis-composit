	...
	if ($arBlog = CBlog::GetByUrl($arParams["BLOG_URL"]))
	{
		if($arBlog["ACTIVE"] == "Y")
		{
			...
			if($arGroup["SITE_ID"] == SITE_ID)
			{
				...
				$arUserGroups = CBlogUser::GetUserGroups(($USER->IsAuthorized() ? $USER->GetID() : 0), $arBlog["ID"], "Y", BLOG_BY_USER_ID);
				...
				else
				{
					...
					$dateFrom = mktime(0, 0, 0, $arParams["MONTH"], 1, $arParams["YEAR"]);
					$dateTo = mktime(0, 0, 0, $arParams["MONTH"] + 1, 1, $arParams["YEAR"]);
