...
?>
		<div class="forum-topic-move-buttons">
			...
			<select name="newFID" class="forums">
<?
	foreach ($arResult["GROUPS_FORUMS"] as $key => $res):
		if ($res["TYPE"] == "GROUP"):
?>
				<optgroup label="<?=($res["DEPTH"] > 0 ? str_pad("", ($res["DEPTH"] - 1)*24, "&nbsp;") : "").$res["NAME"]
				?>" class="forums level<?=$res["DEPTH"]?>"></optgroup>
<?
		else:
?>
				<option value="<?=$res["ID"]?>" <?=($arParams["TID"] == $res["ID"] ? "selected='selected'" : "")?> <?
					?>class="forums level<?=$res["DEPTH"]?>"><?=($res["DEPTH"] > 0 ? str_pad("", ($res["DEPTH"] + 1)*24, "&nbsp;") : "").$res["NAME"]?></option>
<?
		endif;
	endforeach;
?>
