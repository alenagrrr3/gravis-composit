...
********************************************************************/
	...
	if (!empty($arParams["FID_RANGE"]) && (!$USER->IsAdmin() || $arParams["SHOW_FORUMS_LIST"] == "Y")):
...
/*******************************************************************/
	...
	while ($res = $dbForum->GetNext())
	{
		...
		$res["MODERATE"] = array("TOPICS" => 0, "POSTS" => 0);
		$res["mCnt"] = intVal(intVal($res["mCnt"]) <= 0 ? 0 : $res["mCnt"]);
		if ($res["mCnt"] <= 0)
			$res["mCnt"] = intVal(intVal($res["MCNT"]) <= 0 ? "" : $res["MCNT"]);
		if ($res["mCnt"] > 0 && $res["PERMISSION"] >= "Q"):
			$res["MODERATE"]["TOPICS"] = 0; // for the future
			$res["MODERATE"]["POSTS"] = $res["mCnt"];
		endif;
		...
		$res["URL"] = array(
			...
			"MESSAGE" => CComponentEngine::MakePathFromTemplate($arParams["URL_TEMPLATES_MESSAGE"], 
				array("FID" => $res["ID"], "TID" => $res["TID"], "MID" => $res["MID"]))."#message".$res["MID"], 
...
/*******************************************************************/
	...
	foreach ($arGroupForum as $PARENT_ID => $res)
	{
		...
		while ($PARENT_ID >= $arParams["GID"] && $PARENT_ID != 0) 
		{
