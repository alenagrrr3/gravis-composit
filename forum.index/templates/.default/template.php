...
?>
		<thead>
			<tr>
				<th class="forum-column-title" colspan="2" scope="col"><div class="forum-head-title"><span><?=$arGroup["~NAME"]?></span></div></th>
...
?>
				...
				if ($res["NewMessage"] == "Y" && strLen($arParams["TMPLT_SHOW_ADDITIONAL_MARKER"]) > 0):
								?><a href="<?=$res["URL"]["TOPICS"]?>" class="forum-new-message-marker"><?=$arParams["TMPLT_SHOW_ADDITIONAL_MARKER"]?></a><?
				endif;
								?></span></div>
...
?>
							...
							if ($res["MODERATE"]["POSTS"] > 0):
								?><?=GetMessage("F_NOT_APPROVED_POSTS")?>:&nbsp;<span><a href="<?=$res["URL"]["MODERATE_MESSAGE"]?>"><?=$res["MODERATE"]["POSTS"]?></a></span><?
							endif;
				endif;
?>
						</div>
...
?>
						<div class="forum-lastpost-box">
							<span class="forum-lastpost-title"><?
								?><a href="<?=$res["URL"]["MESSAGE"]?>" title="<?=htmlspecialcharsEx($res["~TITLE"]." (".$res["~LAST_POSTER_NAME"].")")?>"><?
									?><?=$res["TITLE"]?> <span class="forum-lastpost-author">(<?=$res["LAST_POSTER_NAME"]?>)</span></a></span>
...
?>
					...
					<td class="forum-column-title">
						<div class="forum-item-info">
							<div class="forum-item-name"><span class="forum-item-title"><a href="<?=$arResult["URL"]["GROUP_".$res["ID"]]?>"><?
								?><?=$res["~NAME"];?></a></span></div>
				...
				if (array_key_exists("FORUMS", $res)):
					...
					foreach ($res["GROUPS"] as $val):
						...
						?><a href="<?=$arResult["URL"]["GROUP_".$val["ID"]]?>"><?=$val["~NAME"]?></a><?
						$bFirst = false;
					endforeach;
				endif;
...
?>
							...
							if ($res["MODERATE"]["POSTS"] > 0):
								?><?=GetMessage("F_NOT_APPROVED_POSTS")?>:&nbsp;<span><a href="<?=$arResult["URL"]["GROUP_".$res["ID"]]?>"><?
									?><?=$res["MODERATE"]["POSTS"]?></a></span><?
							endif;
				endif;
?>
						</div>
...
<?
		if ($arParams["SHOW_RSS"] == "Y"):
?>
							<span class="forum-footer-option forum-footer-rss forum-footer-option-first"><a href="<?=$arResult["URL"]["RSS"]?>"><?
...
?>
							<span class="forum-footer-option forum-footer-markread<?=($arParams["SHOW_RSS"] == "Y" ? "" : " forum-footer-option-first")?>"><?
								?><a href="<?=$APPLICATION->GetCurPageParam("ACTION=SET_BE_READ&".bitrix_sessid_get(), array("ACTION", "sessid"))?>"><?
									?><?=GetMessage("F_SET_FORUMS_READ")?></a></span>
<?		
