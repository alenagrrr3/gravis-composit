...
if($obSearch->errorno==0)
{
	...
	while($arSearch = $obSearch->GetNext())
	{
		...
		{
			$arSearch["URL"] = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER"], array("user_id" => substr($arSearch["ITEM_ID"], 1)));
		}
	...
	if(count($arResult["SEARCH_RESULT"])>0)
	{
		if(strlen($arResult["~tags"])>0)
			$arResult["ORDER_LINK"] = $APPLICATION->GetCurPageParam("tags=".$arResult["tags"]."&where=".$arResult["where"], Array("tags", "where", "how"));
		else
			$arResult["ORDER_LINK"] = $APPLICATION->GetCurPageParam("q=".$arResult["q"]."&where=".$arResult["where"], Array("q", "where", "how"));
		if($arResult["~how"]!="d")
